...
//!   than QQ (e.g. this tool itself while reading the DB) 鈥?callers filter by
//!   process name.
...
mod windows {
    ...
    fn describe_err(label: &str, err: u32) -> String {
        let hint = match err {
            ...
            ERROR_ACCESS_DENIED => {
                " (cannot enumerate holders without permission 鈥?if QQ runs \
                 elevated, this tool must too)"
                    .to_string()
            }
            ERROR_SESSION_CREDENTIAL_CONFLICT => {
                " (session credential conflict 鈥?cannot enumerate processes in \
                 this session)"
                    .to_string()
            }
            _ => String::new(),
...
mod unix {
    ...
    pub fn probe_db_lock(db_path: &Path) -> io::Result<Vec<DbHolder>> {
        ...
        // whether a conflicting lock exists 鈥?it acquires nothing. `fl` is a
